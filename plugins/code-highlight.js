...
// 复制
import Clipboard from "_clipboard@2.0.4@clipboard";
...
Vue.directive('code-highlight', (el) => {
  ...
  highlight.forEach((block) => {
    ...
    pre_head.innerHTML = "<p><span class='code-language'>" + language.toUpperCase() + "</span><span class='copy'><i class='tag icon'></i>复制</span></p>";
    block.parentNode.insertBefore(pre_head, block)
  })
  // 复制操作
  el.querySelectorAll('pre').forEach((item)=>{
    JQ(item).find('.copy').click((e)=>{
